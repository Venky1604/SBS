...
if uploaded_file:
    ...
    st.info("No file uploaded. Using Breast Cancer Wisconsin dataset as default.")
...
with st.sidebar:
    ...
    if target_col:
        k_max = len(df.columns) - 1
        k = st.slider("Select number of features to keep", 1, k_max, k_max // 2)
        if st.button("Run SBS"):
            with st.spinner("Running SBS analysis..."):
                ...
                selected_features = df.drop(columns=[target_col]).columns[selected_indices]
                new_df = df[selected_features.tolist() + [target_col]]
